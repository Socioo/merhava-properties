    ...
    try {
      ...
      if (data) {
        setFormData({
          ...
          plotSize: data.land_details?.plotSize || '',
          zoning: data.land_details?.zoning || '',
          topography: data.land_details?.topography || '',
          accessRoad: data.land_details?.accessRoad ?? true,
          utilities: Array.isArray(data.land_details?.utilities) ? data.land_details.utilities.join(', ') : '',
        });
      }
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.category !== 'land') {
        propertyData.bedrooms = formData.bedrooms ? parseInt(formData.bedrooms) : null;
        propertyData.bathrooms = formData.bathrooms ? parseFloat(formData.bathrooms) : null;
        propertyData.land_details = null;
      }
  ...
  if (loadingData) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <p style={{ color: '#7f8c8d' }}>Loading property data...</p>
      </div>
    );
  }
